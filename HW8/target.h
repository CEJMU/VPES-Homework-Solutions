...
public:
    ...
    void printBuffer(unsigned int max, unsigned int n)
    {
        ...
        const std::string empty(max - n, ' ');

        std::cout << "\033[1;35m"
                  << std::format("({})@{:>12}: Target Buffer: [{}{}] ( {} / {} )",
                                 name(),
    ...
    void sendEndRequest(tlm::tlm_generic_payload & trans)
    {
        ...
        sc_core::sc_time delay = sc_core::sc_time(ACCEPT_DELAY_NS, sc_core::SC_NS);

        tlm::tlm_sync_enum status = tSocket->nb_transport_bw(trans, bw_phase, delay); // [1.2]
        // Ignore return value (has to be TLM_ACCEPTED anyway)
        // initiator cannot terminate transaction at this point

        // Queue internal event to mark beginning of response
        delay = delay + sc_core::sc_time(INTERNAL_DELAY_NS, sc_core::SC_NS);
    ...
    void executeTransaction(tlm::tlm_generic_payload & trans)
    {
        ...
        std::cout << "\033[1;32m"
                  << std::format("({})@{:>12}: Exec. {} Addr = {:08d} Data = 0x{:08X}",
                                 name(),
                                 sc_core::sc_time_stamp().to_string(),
    ...
    void sendResponse(tlm::tlm_generic_payload & trans)
    {
        ...
        tlm::tlm_sync_enum status = tSocket->nb_transport_bw(trans, bw_phase, delay); // [1.4]
