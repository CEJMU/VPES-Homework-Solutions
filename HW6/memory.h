#pragma once

#include <systemc>
#include <tlm>

template <unsigned int N = 1024> class memory : sc_core::sc_module, tlm::tlm_fw_transport_if<>
{
private:
    std::array<unsigned char, N> mem;

public:
    tlm::tlm_target_socket<> tSocket;

    SC_CTOR(memory) : tSocket("tSocket") { tSocket.bind(*this); }

    void b_transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& delay)
    {
        if (trans.get_address() >= N)
        {
            SC_REPORT_FATAL(this->name(), "Out of Range");
        }

        if (trans.get_command() == tlm::TLM_WRITE_COMMAND)
        {
            std::memcpy(&mem[trans.get_address()], trans.get_data_ptr(), trans.get_data_length());
        }
        else // (trans.get_command() == tlm::TLM_READ_COMMAND)
        {
            std::memcpy(trans.get_data_ptr(), &mem[trans.get_address()], trans.get_data_length());
        }

        delay += sc_core::sc_time(20, sc_core::SC_NS);
    }

    // Dummy method
    virtual tlm::tlm_sync_enum
    nb_transport_fw(tlm::tlm_generic_payload& trans, tlm::tlm_phase& phase, sc_core::sc_time& delay)
    {
        SC_REPORT_FATAL(this->name(), "nb_transport_fw is not implemented");
        return tlm::TLM_ACCEPTED;
    }

    // Dummy method
    bool get_direct_mem_ptr(tlm::tlm_generic_payload& trans, tlm::tlm_dmi& dmi_data)
    {
        SC_REPORT_FATAL(this->name(), "get_direct_mem_ptr is not implemented");
        return false;
    }

    // Dummy method
    unsigned int transport_dbg(tlm::tlm_generic_payload& trans)
    {
        SC_REPORT_FATAL(this->name(), "transport_dbg is not implemented");
        return 0;
    }
};
